...
class ChatHistory(BaseModel):
    ...
    def inject(self, item: ChatHistoryItem | Mapping[str, Role | str]) -> None:
        ...
        if isinstance(item, ChatHistoryItem):
            ...
            # Handle ChatHistoryItem-like objects that might have different class paths
            item = ChatHistoryItem(role=item.role, content=item.content)
        else:
...
class Session(BaseModel):
    ...
    task_metadata: Optional[dict[str, Any]] = None  # Metadata for the task
    output_dir: Optional[str] = None  # Directory to save evaluation details
